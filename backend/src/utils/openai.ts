...
if (!config.openaiKey) throw new Error("OPENAI_API_KEY is missing");
...
// ============================================
// NEW: Conversational Dietary Consultant Chat
// ============================================
...
export async function generateRandomUserPrompts(): Promise<string[]> {
    try {
        const response = await openai.chat.completions.create({
            model: 'gpt-4o-mini',
            messages: [
                ...
                },
                {
                    role: 'user',
                    content:
                        'Generate random short dietary question examples as a JSON array of strings.',
                },
            ],
            response_format: { type: 'json_object' },
            temperature: 0.9,
